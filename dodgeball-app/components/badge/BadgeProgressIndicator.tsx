...
import { type BadgeProgressData } from '@/lib/badgeProgress';
...
/**
 * 배지 진행도 표시 컴포넌트
 * 학생의 다음 배지 획득까지의 진행 상황을 보여줌
 *
 * @param progressData - 전체 진행도 데이터 (카테고리별 바로 다음 배지만 포함)
 * @param showEmpty - 진행도가 없을 때도 메시지 표시 여부
 */
export function BadgeProgressIndicator({
    ...
    if (showEmpty) {
      return (
        <div className="flex items-center gap-2 text-xs text-gray-400 px-1 py-0.5">
          <span>😅</span>
          <span className="text-gray-500">다음 배지 없음</span>
        </div>
      );
    }
    return null;
  ...
  // 카테고리별 진행도 (이미 각 카테고리당 1개씩만 온 상태)
  const categoryProgress = progressData.slice(0, 5);
        ...
        >
          ...
          >
            ...
            <div className="space-y-3">
              {categoryProgress.map((progress, idx) => (
